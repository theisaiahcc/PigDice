...
function passTurn() {
    ...
    if (playerOne.classList.contains("active")) {
        ...
        if (scoreWins(currScore)) get("message").innerText = "Player 1 wins!";
        else nextTurn();
    } else {
        var currScore = parseInt(get("score-two").innerText);
        currScore += total;
        get("score-two").innerText = "" + currScore;
        if (scoreWins(currScore)) get("message").innerText = "Player 2 wins!";
        else nextTurn();
    }
}
function nextTurn() {
...
//# sourceMappingURL=index.0b218e4f.js.map
